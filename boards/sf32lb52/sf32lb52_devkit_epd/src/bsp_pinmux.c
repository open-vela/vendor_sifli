...
void BSP_PIN_LCD(void)
{
    ...
    // LCDC1 - SPI
    HAL_PIN_Set(PAD_PA02, GPIO_A2, PIN_PULLUP, 1);      // EPD BUSY
    HAL_PIN_Set(PAD_PA03, LCDC1_SPI_CS, PIN_NOPULL, 1);
    HAL_PIN_Set(PAD_PA04, LCDC1_SPI_CLK, PIN_NOPULL, 1);
    HAL_PIN_Set(PAD_PA05, LCDC1_SPI_DIO0, PIN_NOPULL, 1);
    HAL_PIN_Set(PAD_PA06, LCDC1_8080_DC, PIN_NOPULL, 1);  // EPD D/CX
    HAL_PIN_Set(PAD_PA10, GPIO_A10, PIN_NOPULL, 1);     // EPD LDO enable, high active
    BSP_GPIO_Set(10, 1, 1);
#else
