...
#define LCD_DRIVER_EXPORT(name, id, init_cfg, dev_ops, ic_max_hor_res, ic_max_ver_res, pixel_align)         \
    RT_USED const lcd_drv_desc_t __lcddriver_##name                              \
    __attribute__((section("LcdDriverDescTab"))) =                              \
    {                                                                            \
        #name,        \
        id,           \
        init_cfg,     \
...
#define LCD_DRIVER_DELAY_MS(ms) usleep((useconds_t)(ms) * 1000)
typedef enum
{
    LCD_STATUS_NONE = 0,
    LCD_STATUS_OPENING,
    LCD_STATUS_NOT_FIND_LCD,
    LCD_STATUS_INITIALIZED,
