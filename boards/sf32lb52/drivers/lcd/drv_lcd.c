...
const void *drv_lcd_co5300_ref __attribute__((used)) = &__lcddriver_co5300;
#endif
...
static void (*drv_lcd_tx_complete)(rt_device_t dev, void *buffer);

int drv_lcd_set_tx_complete(void (*cb)(rt_device_t dev, void *buffer))
{
    drv_lcd_tx_complete = cb;
    return 0;
}
...
static int lcd_task_entry(int argc, FAR char *argv[]);
...
{
    uint32_t old_state;
    rt_base_t mask = (rt_base_t)enter_critical_section();
    ...
    leave_critical_section((irqstate_t)mask);
...
{
    sifli_gpio_config(QAD_SPI_USE_GPIO_CS, GPIO_OUTPUT);
    sifli_gpio_write(QAD_SPI_USE_GPIO_CS, true);
}

void gpio_cs_enable(void)
{
    sifli_gpio_write(QAD_SPI_USE_GPIO_CS, false);
}
...
#define MAX_LCD_API_TIME  (10*1000)  /*10 seconds should be enough for LCD initialization.*/
    err = nxsem_tickwait_uninterruptible(&p_drvlcd->sem.sem,
                                         MSEC2TICK(MAX_LCD_API_TIME));
    if (err == 0) p_drvlcd->sem.value = 0;
...
static void api_unlock(LCD_DrvTypeDef *p_drvlcd)
{
    ...
    err = nxsem_post(&p_drvlcd->sem.sem);
    if (err == 0) p_drvlcd->sem.value = 1;
...
static void print_all_msgs(rt_mq_t mq)
{
    ...
    {
        uint32_t index = (mq->head + i) % mq->depth;
        LCD_MsgTypeDef *msg = (LCD_MsgTypeDef *)(mq->buffer + index * mq->msg_size);
        LOG_E("[%lu] id:%d,tick:%lu,drv:%p", (unsigned long)i, msg->id,
              (unsigned long)msg->tick, msg->driver);
    }
}
...
static rt_err_t send_msg_to_lcd_task(LCD_MsgTypeDef *msg)
{
    ...
    if (p_drvlcd->mq == NULL || sizeof(*msg) > p_drvlcd->mq->msg_size)
    {
        err = -EINVAL;
    }
    else
    {
        err = nxsem_trywait(&p_drvlcd->mq->slots);
        if (err < 0)
        {
            err = -RT_ETIMEOUT;
        }
        ...
        {
            nxsem_wait_uninterruptible(&p_drvlcd->mq->lock);
            memcpy(p_drvlcd->mq->buffer +
                   p_drvlcd->mq->tail * p_drvlcd->mq->msg_size,
                   msg, p_drvlcd->mq->msg_size);
            p_drvlcd->mq->tail =
                (p_drvlcd->mq->tail + 1) % p_drvlcd->mq->depth;
            p_drvlcd->mq->entry++;
            nxsem_post(&p_drvlcd->mq->lock);
            nxsem_post(&p_drvlcd->mq->items);
            err = RT_EOK;
        }
    }
    ...
    else
    {
        if (IS_SYNC_MSG_ID(msg->id))
        {
            ...
            err = nxsem_wait_uninterruptible(&p_drvlcd->sync_msg_sem.sem);
            if (err == 0) p_drvlcd->sync_msg_sem.value = 0;
            RT_ASSERT(RT_EOK == err);
...
{
    LCD_DrvTypeDef *p_drvlcd = container_of(lcdc, LCD_DrvTypeDef, hlcdc);
    p_drvlcd->debug_cnt2++;
    ...
    if (lcdc->XferCpltCallback != NULL)
    {
        ...
        err = nxsem_post(&p_drvlcd->draw_sem.sem);
        if (err == 0) p_drvlcd->draw_sem.value = 1;
        RT_ASSERT(RT_EOK == err);
    }
...
static rt_err_t api_lcd_init(rt_device_t dev)
{
    ...
    nxsem_init(&drv_lcd.sem.sem, 0, 1);
    drv_lcd.sem.value = 1;

    drv_lcd.mq = calloc(1, sizeof(*drv_lcd.mq));
    if (drv_lcd.mq != NULL)
    {
        drv_lcd.mq->buffer = calloc(4, sizeof(LCD_MsgTypeDef));
        if (drv_lcd.mq->buffer != NULL)
        {
            drv_lcd.mq->msg_size = sizeof(LCD_MsgTypeDef);
            drv_lcd.mq->depth = 4;
            nxsem_init(&drv_lcd.mq->slots, 0, drv_lcd.mq->depth);
            nxsem_init(&drv_lcd.mq->items, 0, 0);
            nxsem_init(&drv_lcd.mq->lock, 0, 1);
        }
        else
        {
            free(drv_lcd.mq);
    ...
    priority = 6;
#endif
    drv_lcd.task.name = "lcd_task";
    drv_lcd.task.entry = lcd_task;
    drv_lcd.task.parameter = drv_lcd.mq;
    drv_lcd.task.priority = priority;
    drv_lcd.task.stack_size = sizeof(drv_lcd_stack);
    drv_lcd.task.pid = kthread_create(drv_lcd.task.name, priority,
                                      sizeof(drv_lcd_stack),
                                      lcd_task_entry, NULL);
    RT_ASSERT(drv_lcd.task.pid >= 0);
    nxsem_init(&drv_lcd.draw_sem.sem, 0, 0);
    drv_lcd.draw_sem.value = 0;
    nxsem_init(&drv_lcd.sync_msg_sem.sem, 0, 0);
    drv_lcd.sync_msg_sem.value = 0;
#ifdef COPY2COMPRESS_FB_AND_SEND
...
{
    return drv_lcd.task.name != NULL ? &drv_lcd.task : NULL;
}
...
#ifdef LCD_USE_GPIO_TE
        sifli_gpio_config(LCD_USE_GPIO_TE, GPIO_INPUT);
        sifli_gpio_set_event(LCD_USE_GPIO_TE, true, false,
                             te_pin_irq_handler, &drv_lcd.hlcdc);
#endif /* LCD_USE_GPIO_TE */
...
#ifdef LCD_USE_GPIO_TE
    sifli_gpio_set_event(LCD_USE_GPIO_TE, false, false, NULL, NULL);
#endif /* LCD_USE_GPIO_TE */
...
static void lcd_driver_print_error_info(void)
{
    ...
    if (drv_lcd.hlcdc.Instance)
    {
        ...
        rt_base_t mask;
        mask = (rt_base_t)enter_critical_section();
        leave_critical_section((irqstate_t)mask);
...
static void aysnc_send_cmplt_cbk(LCD_DrvTypeDef *p_drvlcd, const uint8_t *buffer)
{
    ...
    if (drv_lcd_tx_complete)
    {
        drv_lcd_tx_complete(p_lcd_device, (void *)buffer);
    }
...
static rt_err_t api_lcd_control(rt_device_t dev, int cmd, void *args)
{
    ...
    if (RTGRAPHIC_CTRL_GET_BUSY == cmd)
    {
        if (args)
        {
            ...
            result = nxsem_trywait(&drv_lcd.sem.sem);
            if (result == 0) drv_lcd.sem.value = 0;
            if (result == -EAGAIN) result = -RT_ETIMEOUT;
            ...
            else if (result == RT_EOK)
            {
                ...
                result = nxsem_post(&drv_lcd.sem.sem);
                if (result == 0) drv_lcd.sem.value = 1;

                /*Check async DMA copy*/
#if defined(COPY2COMPRESS_FB_AND_SEND) && !defined(BSP_USE_LCDC2_ON_HPSYS)
                result = nxsem_trywait(&compress_fb_ctx.dma_sema.sem);
                if (result == 0) compress_fb_ctx.dma_sema.value = 0;
                if (result == -EAGAIN) result = -RT_ETIMEOUT;
                if (result == -RT_ETIMEOUT)
                {
                    *(bool *)args = true;
                ...
                {
                    result = nxsem_post(&compress_fb_ctx.dma_sema.sem);
                    if (result == 0) compress_fb_ctx.dma_sema.value = 1;
                }
                else
                {
                    RT_ASSERT(0);
...
#error "Can't define both 'ROUND_TYPE_LCD' and 'RECT_TYPE_LCD'"
    ...
    default:
        lcd_layer_control(&drv_lcd, cmd, args);
        break;
    }
...
static rt_err_t draw_core(LCD_DrvTypeDef *p_drvlcd, const uint8_t *pixels, int x0, int y0, int x1, int y1)
{
    ...
    else if (p_drvlcd->p_drv_ops->p_ops->WriteMultiplePixels)
    {
        ...
        {
            uint32_t cur_tick = (uint32_t)clock_systime_ticks();
            //Check ESD every 3 seconds
            if (HAL_GetElapsedTick(p_drvlcd->last_esd_check_tick, cur_tick) > MSEC2TICK(3000))
            {
        ...
        if (RT_EOK == err)
        {
            ...
            //enter_critical_section();//In case of task switch between 'get tick' and 'WriteMultiplePixels'
            p_drvlcd->start_tick = (uint32_t)clock_systime_ticks();
            p_drvlcd->p_drv_ops->p_ops->WriteMultiplePixels(&p_drvlcd->hlcdc, pixels, new_x0, new_y0, new_x1, new_y1);
            //leave_critical_section();

            /* --------- Wait send compelete -----------------*/
            err = nxsem_tickwait_uninterruptible(&p_drvlcd->draw_sem.sem,
                                                 MAX_LCD_DRAW_TIME);
            if (err == 0) p_drvlcd->draw_sem.value = 0;
        ...
        if (RT_EOK == err)
        {
            ...
            p_drvlcd->end_tick = (uint32_t)clock_systime_ticks();
        ...
        else if (-RT_ETIMEOUT == err)
        {
            ...
            while (nxsem_trywait(&p_drvlcd->draw_sem.sem) == 0) {}
            p_drvlcd->draw_sem.value = 0;
            rt_err_t err2 = nxsem_post(&p_drvlcd->draw_sem.sem);
            RT_ASSERT(RT_EOK == err2);

            p_drvlcd->draw_error = 1;
            p_drvlcd->statistics.draw_core_err_cnt++;
            lcd_driver_print_error_info();
            usleep(30 * 1000);//Wait LCDC finish if it was just started.
            lcd_driver_print_error_info();
        ...
        if ((uint32_t)clock_systime_ticks() - p_drvlcd->statistics.start_tick  > 2 * RT_TICK_PER_SECOND)
        {
            if (p_drvlcd->statistics_log)
            {
                uint32_t fps = 0;
                uint32_t seconds = ((uint32_t)clock_systime_ticks() - p_drvlcd->statistics.start_tick) / RT_TICK_PER_SECOND;
            ...
            p_drvlcd->statistics.start_tick = (uint32_t)clock_systime_ticks();
        }
        ...
        if (drv_lcd.p_drv_ops->p_ops->SetRegion)
        {
            ...
            if (drv_lcd.send_time_log)
            {
                LOG_I("gap %02d ms, set_window x0x1y0y1[%d,%d,%d,%d]",
                      ((uint32_t)clock_systime_ticks() - drv_lcd.end_tick) * 1000 / RT_TICK_PER_SECOND, //elapsed ms util last draw_core done.
                      x0, x1, y0, y1);
            }
            new_x0 = x0;
            new_x1 = x1;
...
static void lcd_task(void *param)
{
    ...
    while (1)
    {
        ...
        if (msg_queue == NULL || sizeof(msg) < msg_queue->msg_size)
        {
            err = -EINVAL;
        }
        else
        {
            err = nxsem_wait_uninterruptible(&msg_queue->items);
            if (err == 0)
            {
                nxsem_wait_uninterruptible(&msg_queue->lock);
                memcpy(&msg,
                       msg_queue->buffer +
                       msg_queue->head * msg_queue->msg_size,
                       msg_queue->msg_size);
                msg_queue->head =
                    (msg_queue->head + 1) % msg_queue->depth;
                msg_queue->entry--;
                nxsem_post(&msg_queue->lock);
                nxsem_post(&msg_queue->slots);
            }
        }
        ...
        start_tick = (uint32_t)clock_systime_ticks();
        ...
        if (RT_EOK == err)
        {
            ...
            case LCD_MSG_OPEN:
            {
                ...
                if (drv_lcd.p_drv_ops)
                {
                    drv_lcd.buf_format = drv_lcd.hlcdc.Init.color_mode;
                    HAL_LCDC_LayerSetFormat(&drv_lcd.hlcdc,
                                            drv_lcd.select_layer,
                                            drv_lcd.hlcdc.Init.color_mode);
                    set_drv_lcd_state(LCD_STATUS_INITIALIZED);
                }
                else
                {
            ...
            case LCD_MSG_DRAW_COMP_RECT_ASYNC:
            {
                ...
                err = nxsem_tickwait_uninterruptible(&compressed_buffer->sema.sem,
                                                     MSEC2TICK(MAX_LCD_DRAW_TIME));
                if (err == 0) compressed_buffer->sema.value = 0;
                ...
                if (RT_EOK == err)
                {
                    ...
                    */
                    ...
                    err = nxsem_post(&compressed_buffer->sema.sem);
                    if (err == 0) compressed_buffer->sema.value = 1;
                    RT_ASSERT(RT_EOK == err);
                }
                else
                {
                    LOG_E("%d, skiped compressed_buf %x, elapsed %d ticks", err, compressed_buffer->addr,
                          (uint32_t)clock_systime_ticks() - compressed_buffer->start_tick);
            ...
            case LCD_MSG_FLUSH_RECT_ASYNC:
            {
                ...
                HAL_LCDC_LayerSetFormat(&p_drvlcd->hlcdc, p_drvlcd->select_layer,
                                        lcd_format_to_hal_lcd_format(flush_info->color_format));
        ...
        if (IS_SYNC_MSG_ID(msg.id))
        {
            err = nxsem_post(&p_drvlcd->sync_msg_sem.sem);
            if (err == 0) p_drvlcd->sync_msg_sem.value = 1;
            RT_ASSERT(RT_EOK == err);
        }

        LOG_D("lcd_task exec msg%x: [%s] cost=%dms.", msg.tick, lcd_msg_to_name(msg.id),
              (uint32_t)clock_systime_ticks() - start_tick);
    }

}

static int nuttx_lcd_getvideoinfo(FAR struct lcd_dev_s *dev,
                                  FAR struct fb_videoinfo_s *vinfo)
{
    struct rt_device_graphic_info info;

    if ((dev == NULL) || (vinfo == NULL)) return -EINVAL;
    exe_lcd_control_cmds(dev, RTGRAPHIC_CTRL_GET_INFO, &info);
    if (info.bits_per_pixel == 0) info.bits_per_pixel = 16;
    memset(vinfo, 0, sizeof(*vinfo));
    vinfo->fmt = (info.bits_per_pixel == 16) ? FB_FMT_RGB16_565 :
                 (info.bits_per_pixel == 24) ? FB_FMT_RGB24 : FB_FMT_RGB32;
    vinfo->xres = info.width;
    vinfo->yres = info.height;
    vinfo->nplanes = 1;
    return 0;
}

static int nuttx_lcd_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row,
                            fb_coord_t col, FAR const uint8_t *buffer,
                            size_t npixels)
{
    UNUSED(dev);
    if ((buffer == NULL) || (npixels == 0)) return -EINVAL;
    api_lcd_set_window(col, row, col + npixels - 1, row);
    api_lcd_draw_rect((const char *)buffer, col, row,
                      col + npixels - 1, row);
    return 0;
}

static int nuttx_lcd_putarea(FAR struct lcd_dev_s *dev,
                             ...
                             FAR const uint8_t *buffer, fb_coord_t stride)
{
    ...
    if ((dev == NULL) || (buffer == NULL) || (row_start > row_end) ||
        (col_start > col_end)) return -EINVAL;
    exe_lcd_control_cmds(dev, RTGRAPHIC_CTRL_GET_INFO, &info);
    line_bytes = (col_end - col_start + 1) * info.bits_per_pixel / 8;
    if ((stride == 0) || (stride == line_bytes))
    {
        api_lcd_set_window(col_start, row_start, col_end, row_end);
        api_lcd_draw_rect((const char *)buffer, col_start, row_start,
                          col_end, row_end);
    }
    else
    {
        ...
        {
            api_lcd_set_window(col_start, row, col_end, row);
            api_lcd_draw_rect((const char *)buffer, col_start, row,
                              col_end, row);
            buffer += stride;
        }
    }
...
static int nuttx_lcd_getplaneinfo(FAR struct lcd_dev_s *dev,
                                  unsigned int planeno,
                                  FAR struct lcd_planeinfo_s *pinfo)
{
    struct rt_device_graphic_info info;

    if ((dev == NULL) || (pinfo == NULL) || (planeno != 0)) return -EINVAL;
    exe_lcd_control_cmds(dev, RTGRAPHIC_CTRL_GET_INFO, &info);
    memset(pinfo, 0, sizeof(*pinfo));
    pinfo->putrun = nuttx_lcd_putrun;
    pinfo->putarea = nuttx_lcd_putarea;
    pinfo->bpp = info.bits_per_pixel != 0 ? info.bits_per_pixel : 16;
    pinfo->dev = dev;
    return 0;
}

static int nuttx_lcd_getpower(FAR struct lcd_dev_s *dev)
{
    UNUSED(dev);
    return (drv_lcd.status == LCD_STATUS_DISPLAY_ON) ? CONFIG_LCD_MAXPOWER : 0;
}

static int nuttx_lcd_setpower(FAR struct lcd_dev_s *dev, int power)
{
    return api_lcd_control(dev, power > 0 ? RTGRAPHIC_CTRL_POWERON :
                          RTGRAPHIC_CTRL_POWEROFF, NULL);
}

static int nuttx_lcd_getcontrast(FAR struct lcd_dev_s *dev)
{
    UNUSED(dev);
    return drv_lcd.brightness;
}

static int nuttx_lcd_setcontrast(FAR struct lcd_dev_s *dev,
                                 unsigned int contrast)
{
    uint8_t brightness = contrast > 100 ? 100 : contrast;
    return api_lcd_control(dev, RTGRAPHIC_CTRL_SET_BRIGHTNESS, &brightness);
}

static int nuttx_lcd_getareaalign(FAR struct lcd_dev_s *dev,
                                  FAR struct lcddev_area_align_s *align)
{
    uint16_t pixel_align = drv_lcd.p_drv_ops != NULL ?
                           drv_lcd.p_drv_ops->pixel_align : 1;
    UNUSED(dev);
    if (align == NULL) return -EINVAL;
    align->row_start_align = pixel_align;
    align->height_align = pixel_align;
    align->col_start_align = pixel_align;
    align->width_align = pixel_align;
    align->buf_align = sizeof(uint32_t);
    return 0;
}

static int nuttx_lcd_ioctl(FAR struct lcd_dev_s *dev, int cmd,
                           unsigned long arg)
{
    return api_lcd_control(dev, cmd, (void *)(uintptr_t)arg);
}

static uint16_t nuttx_lcd_openrefs;

static int nuttx_lcd_open(FAR struct lcd_dev_s *dev)
{
    ...
    if (!initialized)
    {
        ret = api_lcd_init(dev);
        if (ret < 0) return ret;
        initialized = true;
    }

    if (nuttx_lcd_openrefs > 0)
    {
        nuttx_lcd_openrefs++;
        return 0;
    }

    ret = api_lcd_open(dev, 0);
    if (ret >= 0) nuttx_lcd_openrefs = 1;
    return ret;
}

static int nuttx_lcd_close(FAR struct lcd_dev_s *dev)
{
    if (nuttx_lcd_openrefs == 0) return -EINVAL;
    nuttx_lcd_openrefs--;
    if (nuttx_lcd_openrefs > 0) return 0;
    return api_lcd_close(dev);
}
...
static int lcd_hw_init(void)
{
    ...
    drv_lcd.parent.getvideoinfo = nuttx_lcd_getvideoinfo;
    drv_lcd.parent.getplaneinfo = nuttx_lcd_getplaneinfo;
    drv_lcd.parent.getpower = nuttx_lcd_getpower;
    drv_lcd.parent.setpower = nuttx_lcd_setpower;
    drv_lcd.parent.getcontrast = nuttx_lcd_getcontrast;
    drv_lcd.parent.setcontrast = nuttx_lcd_setcontrast;
    drv_lcd.parent.getareaalign = nuttx_lcd_getareaalign;
    drv_lcd.parent.ioctl = nuttx_lcd_ioctl;
    drv_lcd.parent.open = nuttx_lcd_open;
    drv_lcd.parent.close = nuttx_lcd_close;

    if (drv_lcd_irq_initialize() < 0)
    {
        LOG_E("failed to attach LCDC1 IRQ");
        return -EIO;
    }
...
{
    return devno == 0 ? &drv_lcd.parent : NULL;
}
...
static void copy2compress_fb_init(void)
{
    ...
    nxsem_init(&compress_fb_ctx.frame_buf[0].sema.sem, 0, 1);
    compress_fb_ctx.frame_buf[0].sema.value = 1;

#if (COMPRESSED_FRAME_BUF_NUM > 1)
    compress_fb_ctx.frame_buf[1].addr = compress_buf2;
    compress_fb_ctx.frame_buf[1].writetimes = 0;
    nxsem_init(&compress_fb_ctx.frame_buf[1].sema.sem, 0, 1);
    compress_fb_ctx.frame_buf[1].sema.value = 1;
#endif

    nxsem_init(&compress_fb_ctx.dma_sema.sem, 0, 1);
    compress_fb_ctx.dma_sema.value = 1;
...
static void copy2compress_fb_deinit(void)
{
    ...
    nxsem_destroy(&compress_fb_ctx.dma_sema.sem);
    nxsem_destroy(&compress_fb_ctx.frame_buf[0].sema.sem);
#if (COMPRESSED_FRAME_BUF_NUM > 1)
    nxsem_destroy(&compress_fb_ctx.frame_buf[1].sema.sem);
#endif
}
...
static void copy2compress_fb_dma_done_cb(void)
{
    ...
    compressed_buffer->cp_done_tick = (uint32_t)clock_systime_ticks();

#ifndef BSP_USE_LCDC2_ON_HPSYS
    err = nxsem_post(&compress_fb_ctx.dma_sema.sem);
    if (err == 0) compress_fb_ctx.dma_sema.value = 1;
    RT_ASSERT(RT_EOK == err);
...
static frame_buf_t *alloc_frame_buf(void)
{
    ...
    irqstate_t flags = enter_critical_section(); //In case of sem changed if switch to lcd_task
    for (i = 0; i < COMPRESSED_FRAME_BUF_NUM; i++)
    {
        buf = &compress_fb_ctx.frame_buf[i];

        int sem_result = nxsem_trywait(&buf->sema.sem);
        if (sem_result == 0)
        {
            buf->sema.value = 0;
            break;
    ...
    if (COMPRESSED_FRAME_BUF_NUM == i)
    {
        ...
        rt_err_t err = nxsem_tickwait_uninterruptible(&buf->sema.sem,
                                                      MSEC2TICK(MAX_LCD_DRAW_TIME));
        if (err == 0) buf->sema.value = 0;
        if (RT_EOK != err)
        {
            WAIT_SEMA_TIMEOUT();
            return NULL;
...
static void free_frame_buf(frame_buf_t *buf)
{
    ...
    buf->writetimes = 0;
    buf->end_tick = (uint32_t)clock_systime_ticks();
...
static frame_buf_t *copy2compress_fb_async(const char *pixels, int32_t x0, int32_t y0, int32_t x1, int32_t y1)
{
    ...
    err = nxsem_tickwait_uninterruptible(&compress_fb_ctx.dma_sema.sem,
                                          MSEC2TICK(1000));
    if (err == 0) compress_fb_ctx.dma_sema.value = 0;
    RT_ASSERT(RT_EOK == err);
#endif /* BSP_USE_LCDC2_ON_HPSYS */
    ...
    free_buffer->start_tick = (uint32_t)clock_systime_ticks();
...
static int lcd_backlight_pwm_fd = -1;

#ifndef LCD_PWM_BACKLIGHT_PEROID
    #define LCD_PWM_BACKLIGHT_PEROID (10*1000)  /*set pwm peroid default: 100k*/
#endif

static int lcd_backlight_pwm_open(void)
{
    if (lcd_backlight_pwm_fd >= 0)
    {
        return lcd_backlight_pwm_fd;
    }

    lcd_backlight_pwm_fd = open("/dev/pwm0", O_RDWR);
    if (lcd_backlight_pwm_fd < 0)
    {
        int ret = -errno;
        LOG_E("open /dev/pwm0 failed: %d", ret);
        return ret;
    }
...
static ssize_t backligt_get(FAR struct file *filep, FAR char *buffer,
                            size_t size)
{
    ...
    *buffer = (char)lcd_backlight_level;
    fd = lcd_backlight_pwm_open();
    if (fd >= 0 && ioctl(fd, PWMIOC_GETCHARACTERISTICS,
                         (unsigned long)(uintptr_t)&info) >= 0)
    {
#ifdef CONFIG_PWM_MULTICHAN
        LOG_I("pwm_config freq=%lu,duty=%lu", (unsigned long)info.frequency,
              (unsigned long)info.channels[0].duty);
#else
        LOG_I("pwm_config freq=%lu,duty=%lu", (unsigned long)info.frequency,
              (unsigned long)info.duty);
#endif
    }
...
static ssize_t backligt_set(FAR struct file *filep, FAR const char *buffer,
                            size_t size)
{
    struct pwm_info_s info = {0};
    uint8_t percent;
    ...
    info.frequency = 1000000000U / LCD_PWM_BACKLIGHT_PEROID;
#ifdef CONFIG_PWM_MULTICHAN
#ifndef LCD_PWM_BACKLIGHT_CHANEL_NUM
#define LCD_PWM_BACKLIGHT_CHANEL_NUM 1
#endif
    info.channels[0].channel = LCD_PWM_BACKLIGHT_CHANEL_NUM;
    info.channels[0].duty = (ub16_t)((uint32_t)percent * 65535U / 100U);
#if CONFIG_PWM_NCHANNELS > 1
    info.channels[1].channel = -1;
#endif
#else
    info.duty = (ub16_t)((uint32_t)percent * 65535U / 100U);
#endif
    ...
    ret = ioctl(fd, percent == 0 ? PWMIOC_STOP : PWMIOC_START, 0);
    if (ret < 0)
    {
        return -errno;
    }
...
static const struct file_operations lcd_backlight_fops =
{
    .read = backligt_get,
    .write = backligt_set,
};
...
static int lcd_backlight_init(void)
{
    return register_driver("/dev/lcdlight", &lcd_backlight_fops,
                           0666, NULL);
}

INIT_BOARD_EXPORT(lcd_backlight_init);
#ifdef RT_USING_FINSH
static rt_err_t get_lcd_pwm_bglit(int argc, char **argv)
{
    struct file file = {0};
    uint8_t level = 0;
    backligt_get(&file, (FAR char *)&level, 1);
...
#ifdef BSP_USING_RAMLESS_LCD
    rt_base_t level = (rt_base_t)enter_critical_section(); //In case RAMLESS FSM is running
...
static rt_err_t lcd_ctrl(int argc, char **argv)
{
    ...
    else if (strcmp(argv[1], "br") == 0)
    {
        ...
        {
            int fd = open("/dev/lcdlight", O_RDONLY);
            if (fd >= 0)
            {
                uint8_t val;
                if (1 == read(fd, &val, 1))
