...
#  define LV_HOR_RES_MAX CONFIG_LCD_HOR_RES_MAX
#endif

#ifndef LV_VER_RES_MAX
#  define LV_VER_RES_MAX CONFIG_LCD_VER_RES_MAX
#endif
...
#ifdef CONFIG_BSP_USING_LCD_FRAMEBUFFER
#  define BSP_USING_LCD_FRAMEBUFFER
#endif

#ifdef CONFIG_LCD_FB_USING_NONE
#  define LCD_FB_USING_NONE
#endif
#ifdef CONFIG_LCD_FB_USING_ONE_UNCOMPRESSED
#  define LCD_FB_USING_ONE_UNCOMPRESSED
#endif
#ifdef CONFIG_LCD_FB_USING_TWO_UNCOMPRESSED
#  define LCD_FB_USING_TWO_UNCOMPRESSED
#endif
#ifdef CONFIG_LCD_FB_USING_ONE_COMPRESSED
#  define LCD_FB_USING_ONE_COMPRESSED
#endif
#ifdef CONFIG_LCD_FB_USING_TWO_COMPRESSED
#  define LCD_FB_USING_TWO_COMPRESSED
#endif
#ifdef CONFIG_LV_FB_ONE_SCREEN_SIZE
#  define LV_FB_ONE_SCREEN_SIZE
#endif
#ifdef CONFIG_LV_FB_TWO_SCREEN_SIZE
#  define LV_FB_TWO_SCREEN_SIZE
#endif
#ifdef CONFIG_LV_FB_ONE_NOT_SCREEN_SIZE
#  define LV_FB_ONE_NOT_SCREEN_SIZE
#endif
#ifdef CONFIG_LV_FB_TWO_NOT_SCREEN_SIZE
#  define LV_FB_TWO_NOT_SCREEN_SIZE
#endif

#ifndef LV_FB_LINE_NUM
#  ifdef CONFIG_LV_FB_LINE_NUM
#    define LV_FB_LINE_NUM CONFIG_LV_FB_LINE_NUM
#  else
#    define LV_FB_LINE_NUM LV_VER_RES_MAX
#  endif
#endif

#ifndef FB_ALIGN_BYTE
#  define FB_ALIGN_BYTE LV_DRAW_BUF_ALIGN
#endif

#ifndef FB_ALIGNED_HOR_RES
...
#ifndef L1_NON_RET_BSS_SECT_BEGIN
#  define L1_NON_RET_BSS_SECT_BEGIN(name)
#  define L1_NON_RET_BSS_SECT_END
#  define L1_NON_RET_BSS_SECT(name, var) \
    var __attribute__((section(".bss.l1_non_ret_bss_" #name)))
#endif

#ifndef L2_NON_RET_BSS_SECT_BEGIN
#  define L2_NON_RET_BSS_SECT_BEGIN(name)
#  define L2_NON_RET_BSS_SECT_END
#  ifdef CONFIG_BSP_USING_PSRAM
#    define L2_NON_RET_BSS_SECT(name, var) \
      __attribute__((section(".psram_bss_" #name))) var
#  else
#    define L2_NON_RET_BSS_SECT(name, var) \
      __attribute__((section(".bss.l2_non_ret_bss_" #name))) var
#  endif
#endif

#ifndef RT_ALIGN
#  define RT_ALIGN(v, a) (((v) + (a) - 1) & ~((a) - 1))
#endif

#ifndef RT_ALIGN_DOWN
#  define RT_ALIGN_DOWN(v, a) ((v) & ~((a) - 1))
#endif
...
static int device = -1;
static struct lv_lcd_info_s info;
static sem_t lcd_sema;
...
****************************************************/
    ...
    */
    ...
    static FB_TYPE buf2_1[FB_LINE_SIZE * LV_VER_RES_MAX]
      __attribute__((section(".psram_bss"), aligned(64)));
    L2_NON_RET_BSS_SECT_END

    #if defined(LCD_FB_USING_TWO_UNCOMPRESSED)||defined(LCD_FB_USING_TWO_COMPRESSED)
        L2_NON_RET_BSS_SECT_BEGIN(frambuf)
        static FB_TYPE buf2_2[FB_LINE_SIZE * LV_VER_RES_MAX]
          __attribute__((section(".psram_bss"), aligned(64)));
        L2_NON_RET_BSS_SECT_END
...
static void render_start(lv_event_t *e)
{
    ...
    if (disp->inv_p > 0)
    {
        ...
        _lv_inv_area(disp, &scr_area); //To avoid copying PSRAM->PSRAM
    }
#endif
...
#endif /* BSP_USING_LCD_FRAMEBUFFER */
    int err;
    err = nxsem_tickwait_uninterruptible(&lcd_sema,
                                         MSEC2TICK(LCD_FLUSH_EXP_MS));
    DEBUGASSERT(err == 0);
    err = nxsem_post(&lcd_sema);
    DEBUGASSERT(err == 0);
}
...
#else
static int lcd_flush_done(int dev, void *buffer)
{
    LV_UNUSED(dev);
    LV_UNUSED(buffer);
    lv_display_t *p_disp = lcd_flushing_disp_drv;
...
static void lcd_flush(lv_display_t *disp_drv, const lv_area_t *refresh_area, uint8_t *color_p)
{
    ...
    {
        .row_start = clip_area.y0,
        .row_end = clip_area.y1,
        .col_start = clip_area.x0,
        .col_end = clip_area.x1,
        .stride = lv_area_get_width(p_buf_area) * (LV_COLOR_DEPTH / 8),
        .data = color_p,
    };
    err = ioctl(device, LCDDEVIO_PUTAREA,
                (unsigned long)(uintptr_t)&area);
    lcd_flush_done(device, NULL);
#endif /* BSP_USING_LCD_FRAMEBUFFER */
...
void lv_lcd_init(const char *name)
{
    ...
    device = open(name, O_RDWR | O_CLOEXEC);
    DEBUGASSERT(device >= 0);
    if (device < 0)
    {
        return;
    }
    ...
    ret = ioctl(device, LCDDEVIO_GETVIDEOINFO,
    ...
    ret = ioctl(device, LCDDEVIO_GETPLANEINFO,
    ...
    (void)ioctl(device, LCDDEVIO_GETAREAALIGN,
                (unsigned long)(uintptr_t)&align);

    info.width = vinfo.xres;
    info.height = vinfo.yres;
    info.bits_per_pixel = pinfo.bpp;
    info.draw_align = align.width_align != 0 ? align.width_align : 1;

    if (info.bits_per_pixel != LV_COLOR_DEPTH)
    {
        LOG_W("framebuffer color depth(%u) mismatch (LV_COLOR_DEPTH=%u)",
    ...
    lv_display_set_driver_data(disp, (void *)(intptr_t)device);

#ifdef BSP_USING_LCD_FRAMEBUFFER
    drv_lcd_fb_init(name);
    memset(buf2_1, 0, sizeof(buf2_1));
    up_clean_dcache((uintptr_t)buf2_1,
                    (uintptr_t)buf2_1 + sizeof(buf2_1));
#if defined(LCD_FB_USING_TWO_COMPRESSED)||defined(LCD_FB_USING_TWO_UNCOMPRESSED)
    memset(buf2_2, 0, sizeof(buf2_2));
    up_clean_dcache((uintptr_t)buf2_2,
                    (uintptr_t)buf2_2 + sizeof(buf2_2));
#endif
    update_fb();
#endif /* BSP_USING_LCD_FRAMEBUFFER */
...
void lv_nuttx_init_custom(const lv_nuttx_dsc_t *dsc,
                          lv_nuttx_result_t *result)
{
    if (dsc == NULL || result == NULL)
    {
        return;
    }

    lv_lcd_init(dsc->fb_path != NULL ? dsc->fb_path : "/dev/lcd0");
    result->disp = disp;
}
